...
const actions = {
  ...
  async openUserMedia({ state }) {
    ...
    state.callee.remoteStream = new MediaStream();
  },
  ...
  async joinConnectionById({ actions }, connectionId) {
    actions.callee.setConnectionId(connectionId);
    await actions.callee.setConnectionRef(`${connectionId}`);
    await actions.callee.getConnectionSnapshot();
  },
  setConnectionId({ state }, connectionId) {
    state.callee.connectionId = connectionId;
  },
  getConnectionId({ state }) {
    return json(state.callee.connectionId);
  },
  async hangUp({ actions }) {
    // const tracks = document.querySelector("#localVideo").srcObject.getTracks();

    actions.callee.closeUserMedia();
    if (actions.callee.getRemoteStream()) {
      actions.callee
        .getRemoteStream()
        .getTracks()
    ...
    if (actions.callee.getPeerConnection()) {
      actions.callee.getPeerConnection().close();
    }
    if (actions.callee.getConnectionId()) {
      await actions.callee.setConnectionRef();
      ...
      await actions.callee.getConnectionRef().delete();
    }
  },
  async getConnectionSnapshot({ actions }) {
    const connectionSnapshot = await actions.callee.getConnectionRef().get();
    console.log('Got connection:', connectionSnapshot.exists);

    if (connectionSnapshot.exists) {
      // console.log("Create PeerConnection with configuration: ", configuration);
      actions.callee.createPeerConnection();

      // peerConnection = new RTCPeerConnection(configuration);
      // registerPeerConnectionListeners();
      actions.callee.addLocalTracks();
      ...
      const answer = await actions.callee.getPeerConnection().createAnswer();
      console.log('Created answer:', answer);
      await actions.callee.getPeerConnection().setLocalDescription(answer);
      ...
      await actions.callee.getConnectionRef().update(connectionWithAnswer);
  ...
  async setConnectionRef({ state, actions }, connectionId) {
    ...
    if (connectionId) {
      state.callee.connectionRef = await db
        .collection('connections')
        .doc(connectionId);
    } else {
      state.callee.connectionRef = await db.collection('connections').doc();
    }
  },
  getConnectionRef({ state }) {
    return json(state.callee.connectionRef);
  },
  ...
  getRemoteStream({ state }) {
    return json(state.callee.remoteStream);
  },
  getPeerConnection({ state }) {
    return json(state.callee.peerConnection);
  },
  createPeerConnection({ state }) {
    ...
    state.callee.peerConnection = new RTCPeerConnection(configuration);
  },
